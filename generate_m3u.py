...
def generate_m3u(end_episode):
    ...
    for ep in range(1, end_episode + 1):
        ...
        m3u += f"""#EXTINF:-1 tvg-id="karadayı{ep}" tvg-name="Bölüm-{ep}" tvg-logo="{COVER_ART}" group-title="{CATEGORY}",Kara Dayı Bölüm-{ep}
{stream_url}\n"""

    with open("kara_dayı.m3u", "w", encoding="utf-8") as f:
        f.write(m3u)
